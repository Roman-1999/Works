(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".tranding-block__slider")) new core(".tranding-block__slider", {
            modules: [ Navigation, Pagination ],
            observer: false,
            observeParents: false,
            slidesPerView: 4,
    ...
    da.init();
    const script_text = document.querySelector(".right-side__circle-text p");
    script_text.innerHTML = script_text.innerText.split("").map(((char, i) => `<span style="transform:rotate(${i * 16.5}deg)">${char}</span>`)).join("");
    let paths = [ "path1", "path2", "path3", "path4" ];
    let currentPathIndex = 0;
    setInterval((() => {
        document.getElementById(paths[currentPathIndex]).setAttribute("fill", "white");
        currentPathIndex = (currentPathIndex + 1) % paths.length;
        document.getElementById(paths[currentPathIndex]).setAttribute("fill", "#52FFE0");
    }), 800);
    window["FLS"] = true;
    isWebp();
    menuInit();
